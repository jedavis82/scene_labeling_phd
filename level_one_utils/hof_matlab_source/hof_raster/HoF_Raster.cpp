							  	  ...
							  	  int width, int height)
{
	...
	if(abs(typeForce)<=ZERO_FORCE_TYPE)
		methode=7;
	else
		methode=2;
				  	  	  	  ...
				  	  	  	  int Case1, int Case2)
{
  ...
  while (pt_L_A)
    {
      Seg_A=pt_L_A;
      d1=abs(Seg_A->x2-Seg_A->x1)+1;
      /* premier segment */
      pt_L_B=List_Seg_B;
      while (pt_L_B)
	{
	  Seg_B=pt_L_B;
	  d2=abs(Seg_B->x2-Seg_B->x1)+1;
	  ...
	  if (Seg_A->x1 <= Seg_B->x2)
	    {
	      if (Seg_A->x2 < Seg_B->x1)
		{
		  x=abs(Seg_A->x2-Seg_A->x1)+1;
		  z=abs(Seg_B->x2-Seg_B->x1)+1;
		  y=abs(Seg_B->x1-Seg_A->x2)-1;
		...
		if ((Seg_A->x2<Seg_B->x2)&&(Seg_A->x1<Seg_B->x1))
		  ...
		  if ((Seg_B->x1>Seg_A->x1)&&(Seg_A->x2>=Seg_B->x2))
		    ...
		    if ((Seg_B->x1<=Seg_A->x1)&&(Seg_B->x2>Seg_A->x2))
		      {
			x=abs(Seg_A->x2-Seg_A->x1)+1;
			z=abs(Seg_B->x2-Seg_A->x1)+1;
			y=-x;
			During(Histo, x, y, z, Y0, Case1, Poids);
		      }
		    else
		      {
			x=z=abs(Seg_B->x2-Seg_A->x1)+1;
			y=-x;
		...
		if ((Seg_A->x2>Seg_B->x2)&&(Seg_A->x1>Seg_B->x1))
		  ...
		  if ((Seg_A->x1>Seg_B->x1)&&(Seg_B->x2>=Seg_A->x2))
		    ...
		    if ((Seg_A->x1<=Seg_B->x1)&&(Seg_A->x2>Seg_B->x2))
		      {
			z=abs(Seg_B->x2-Seg_B->x1)+1;
			x=abs(Seg_A->x2-Seg_B->x1)+1;
			y=-z;
			Contains(Histo, x, y, z, Y0, Case2, Poids);
		      }
		    else
		      {
			x=z=abs(Seg_A->x2-Seg_B->x1)+1;
			y=-z;
